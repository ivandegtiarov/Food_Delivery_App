...
const AdminOrders = () => {
  ...
  const getOrders = async () => {
    try {
      const { data } = await axios.get("/api/v1/auth/cor-orders");
      setOrders(data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleTakeOrder = async (orderId) => {
    try {
      ...
      await axios.put(`/api/v1/auth/order-statuss/${orderId}`, {
        status: "Доставляется",
      });
      setOrders((prev) =>
        prev.map((order) => (order._id === orderId ? data : order))
      );
      /* setStatus("Доставляется") */
    } catch (error) {
      console.log(error);
    }
  };
...
{/*                   {isOpen[i] && 
                  ...
                  <div className="main_buttons">
                    ...
                    <Link to="/dashboard/courier/progress-orders" className="navbar-brand">
                      <button onClick={() => handleTakeOrder(o?._id)} onChange={() => handleChange(o._id, "Доставляется")} type="button" className="btn btn-success p-10">Взяти замовлення</button>
                    </Link>
                    </div>
                  </div>
